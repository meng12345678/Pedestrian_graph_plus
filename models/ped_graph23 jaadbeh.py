...
#         # 应用卷积得到注意力权重
#         attention = self.conv(combined)  # [N, 1, T, V]
#         attention = self.bn(attention)   # BatchNorm 在这里
#         attention = self.sigmoid(attention)  # 然后 sigmoid
...
#             nn.InstanceNorm1d(hidden_channels),  # 移到这里，在ReLU之前
...
class LayerNorm2d(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 3, 1)
        x = self.norm(x)
        x = x.permute(0, 3, 1, 2)  # 回到 [N, C, H, W]
        return x
class SpatialAttention(nn.Module):
    def __init__(self, channels, dropout=0.1):
        super(SpatialAttention, self).__init__()
        self.conv = nn.Conv2d(2, 1, kernel_size=7, padding=3, bias=False)
        self.bn = nn.GroupNorm(1, 1)  # 更稳定的替代方案
        self.sigmoid = nn.Sigmoid()
        self.dropout = nn.Dropout(dropout)
        self.init_weights()
...
class TemporalAttention(nn.Module):
    ...
    def forward(self, x):
        # x shape: [N, C, T, V]
        x_pooled = torch.mean(x, dim=3)  # [N, C, T]
        att = self.conv1(x_pooled)       # [N, C//4, T]
        att = att.permute(0, 2, 1)       # [N, T, C//4]
        att = self.norm(att)
        att = att.permute(0, 2, 1)       # [N, C//4, T]
        att = self.relu(att)
        att = self.conv2(att)            # [N, C, T]
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        avg_out = self.linear1(avg_out)
        avg_out = self.norm(avg_out)
        avg_out = self.relu(avg_out)
        avg_out = self.linear2(avg_out)

        max_out = self.linear1(max_out)
        max_out = self.norm(max_out)
        max_out = self.relu(max_out)
        max_out = self.linear2(max_out)

        out = self.sigmoid(avg_out + max_out)  # [N, C]
        out = self.dropout(out)
        return out.view(N, C, 1, 1).expand_as(x)
...
class pedMondel(nn.Module):
    ...
    def __init__(self, frames, vel=False, seg=False, h3d=True, nodes=19, n_clss=1):
        ...
        self.data_bn = nn.LayerNorm(self.ch * nodes)
    ...
    def forward(self, kp, frame=None, vel=None):
        ...
        N, C, T, V = kp.shape
        kp = kp.permute(0, 1, 3, 2).contiguous().view(N, C * V, T)  # [N, C*V, T]
        kp = kp.permute(0, 2, 1)  # [N, T, C*V]
        kp = self.data_bn(kp)
        kp = kp.permute(0, 2, 1).contiguous()  # [N, C*V, T]
        kp = kp.view(N, C, V, T).permute(0, 1, 3, 2).contiguous()  # [N, C, T, V]
...
def bn_init(bn, scale):
    if hasattr(bn, 'weight') and hasattr(bn, 'bias'):
        nn.init.constant_(bn.weight, scale)
        nn.init.constant_(bn.bias, 0)
    elif hasattr(bn, 'norm'):  # 如果是自定义封装的 LayerNorm1d/2d
        nn.init.constant_(bn.norm.weight, scale)
        nn.init.constant_(bn.norm.bias, 0)
...
class TCN_GCN_unit(nn.Module): 
    def __init__(self, in_channels, out_channels, A, stride=1, residual=True, adaptive=True, 
        ...
        self.alpha_c = nn.Parameter(torch.tensor(1.0))
    ...
    def forward(self, x):
        ...
        # gcn_out = (
        #         self.alpha_s * gcn_out * spatial_att +
        #         self.alpha_t * gcn_out * temporal_att +
        #         self.alpha_c * gcn_out * channel_att
        # )
        weights = torch.softmax(torch.stack([self.alpha_s, self.alpha_t, self.alpha_c]), dim=0)
        gcn_out = (
                weights[0] * gcn_out * spatial_att +
                weights[1] * gcn_out * temporal_att +
                weights[2] * gcn_out * channel_att
        )
...
# class TCN_GCN_no_Attention(TCN_GCN_unit):
